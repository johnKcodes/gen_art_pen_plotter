...
for i in range(N_POINTS):
    ...
    radius = MAX_RADIUS * angle / (TURNS * 2 * math.pi)

    # Calculate the next point
    x = START_X + radius * math.cos(angle)
    y = START_Y + radius * math.sin(angle)
    noise_contrib = noise.noise2(x/noise_d,y/noise_d)*noise_mult
    noise_contrib *= i/N_POINTS
    radius = MAX_RADIUS * angle / (TURNS * 2 * math.pi) +noise_contrib
